...
export default function SkillsPage() {
  return (
      ...
      <div style={{ borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', marginBottom: '3.5rem' }}>
        {SKILLS.map((group, i) => (
          <motion.div key={group.cat} {...fade(i)}
            className="skill-row" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.25rem', alignItems: 'center', padding: '1rem 0', borderBottom: i < SKILLS.length - 1 ? '1px solid var(--border)' : undefined, transition: 'background 0.18s' }}
    ...
    <style>{`
        ...
        @media (min-width: 480px) { .skill-row { grid-template-columns: 120px 1fr !important; gap: 1.25rem !important; } }
      `}</style>
    </PageLayout>
  )
}
