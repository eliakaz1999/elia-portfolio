...
export default function Hero() {
  ...
  return (
    <section style={{ position: 'relative', width: '100%', minHeight: '100svh', overflow: 'hidden', background: '#000', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Video — muted + playsInline are required HTML attrs for iOS autoplay */}
      <video
        ref={videoRef}
        style={{ position: 'absolute', inset: 0, zIndex: 1, width: '100%', height: '100%', objectFit: 'cover', filter: 'saturate(0.45) brightness(0.55)' }}
        src={VIDEO_URL}
        autoPlay
        loop
        muted
      ...
      <div style={{ position: 'relative', zIndex: 10, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', maxWidth: 1280, width: '100%', margin: '0 auto', padding: '1rem 1.25rem 2.5rem', boxSizing: 'border-box' }}>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.18 }}>
          ...
          {/* Headline */}
          <h1 style={{ fontFamily: "'Instrument Serif', serif", fontWeight: 400, lineHeight: 0.9, letterSpacing: '-0.03em', marginBottom: '1.25rem' }} className="hero-h1">
      ...
      <style>{`
        ...
        .hero-name { font-size: clamp(1.4rem, 5vw, 3rem); }
        /* Hero headline responsive */
        .hero-h1 { font-size: clamp(3.2rem, 10vw, 9rem); }
        /* Use svh for true mobile viewport height */
